// Copyright (c) 2018 The SCU developers
...
CzSCUTracker::CzSCUTracker(std::string strWalletFile)
{
...
CzSCUTracker::~CzSCUTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSCUTracker::Init()
{
...
bool CzSCUTracker::Archive(CMintMeta& meta)
{
...
bool CzSCUTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSCUTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSCUTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSCUTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSCUTracker::GetSerialHashes()
{
...
CAmount CzSCUTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSCUTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSCUTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSCUTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSCUTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSCUTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSCUTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSCUTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSCUTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSCUTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSCUTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSCUTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSCUTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSCUTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSCUTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSCUTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSCUTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
