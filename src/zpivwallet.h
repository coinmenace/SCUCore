...
#ifndef SCU_ZSCUWALLET_H
#define SCU_ZSCUWALLET_H
...
public:
    CzSCUWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSCU(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSCU(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //SCU_ZSCUWALLET_H
