...
CzSCUWallet::CzSCUWallet(std::string strWalletFile)
{
...
bool CzSCUWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
...
void CzSCUWallet::Lock()
{
    seedMaster = 0;
}

void CzSCUWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSCUWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
bool CzSCUWallet::LoadMintPoolFromDB()
{
...
void CzSCUWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSCUWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSCUWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzSCUWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
...
uint512 CzSCUWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSCUWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSCUWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
