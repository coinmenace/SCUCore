...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::SCU)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    QString automintHelp = tr("Current percentage of zSCU.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", true);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in scu.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in scu.conf");
    }
    ...
    bool showzSCUAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzSCUUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzSCUImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzSCUAvailable);
    ui->labelzBalanceMatureText->setVisible(showzSCUAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzSCUUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzSCUUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzSCUImmature);
    ui->labelzBalanceImmatureText->setVisible(showzSCUImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelzSCUPercent->setVisible(showPercentages);
    ui->labelzSCUPercent->setVisible(showPercentages);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("SCU")
    updateDisplayUnit();
}
