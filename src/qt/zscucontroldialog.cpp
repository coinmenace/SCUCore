...
#include "ui_zscucontroldialog.h"
...
std::set<std::string> ZSCUControlDialog::setSelectedMints;
std::set<CMintMeta> ZSCUControlDialog::setMints;

ZSCUControlDialog::ZSCUControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZSCUControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZSCUControlDialog::~ZSCUControlDialog()
{
    delete ui;
}

void ZSCUControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZSCUControlDialog::updateList()
{
...
void ZSCUControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZSCUControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
...
void ZSCUControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
